...
export const list = async (req: AuthRequest, res: Response<ApiResponse>) => {
  ...
  const [items, total] = await Promise.all([
    prisma.expense.findMany({ where, orderBy, skip: (page - 1) * limit, take: limit }),
...
export const getById = async (req: AuthRequest, res: Response<ApiResponse>) => {
  ...
  const item = await prisma.expense.findFirst({ where: { id, userId, isActive: true } });
  if (!item) {
...
export const update = async (req: AuthRequest, res: Response<ApiResponse>) => {
  ...
  const exists = await prisma.expense.findFirst({ where: { id, userId, isActive: true } });
  if (!exists) {
    res.status(404).json({ success: false, message: 'Expense not found' });
    return;
  }
